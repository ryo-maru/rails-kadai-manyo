  ...
  describe 'タスクモデル機能', type: :model do
    ...
    describe 'バリデーションのテスト' do
      ...
      context 'タスクのタイトルと詳細に内容が記載されている場合' do
        it 'バリデーションが通る' do
          task = Task.new(title: '成功テスト', content: '成功', deadline: '2021/07/12', status: '未着手', priority:'中')
  ...
  describe '検索機能' do
    ...
    context 'scopeメソッドでタイトルのあいまい検索とステータス検索をした場合' do
      it "検索キーワードをタイトルに含み、かつステータスに完全一致するタスク絞り込まれる" do
        task = FactoryBot.create(:task, title: 'title1', status: '未着手')
        second_task = FactoryBot.create(:task, title: 'sample', status: '着手中')
        expect(Task.search_by_task_title('title1')).to include(task)
        expect(Task.search_by_status('未着手')).to include(task)
        expect(Task.search_by_task_title('title1').count).to eq 1
        expect(Task.search_by_status('未着手').count).to eq 1
        # ここに内容を記載する
      end
    end
